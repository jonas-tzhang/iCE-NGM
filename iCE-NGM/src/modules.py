...
class KernelDensityEstimation:
    def __init__(self, X, w=None, bandwidth=1):
        self.X = X
        self.w = w
        self.kde = gaussian_kde(X.T, bw_method=bandwidth, weights=w)
    ...
    def weighted_llh(self, X, data_weights=None):
        ...
        """
        data_weights = np.ones(X.shape[0]) if data_weights is None else data_weights
        return np.sum(self.logpdf(X) * data_weights)
    ...
    def __str__(self):
        return "KernelDensityEstimation(X={}, kde={})".format(self.X, self.kde)
