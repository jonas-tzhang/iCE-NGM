...
class ImprovedCEAIS(metaclass=ABCMeta):
    ...
    def check_convergence_fixed_criteria(self, g_X, sigma, delta_target=1.5):
        ...
        # print("Iteration {}, delta = {}".format(len(self.delta), delta))
...
class ImprovedCEAISSingleGaussian(ImprovedCEAIS):
    def initialize_is_density(self):
        """ This function initializes the importance sampling density. """
    ...
    def update_is_density(self, X, g_X, sigma):
        """ This function updates the importance sampling density. """
        resp = self.is_density.responsibility(X)
        W = norm.cdf(-g_X / sigma) * self.input_density.pdf(X) / self.is_density.pdf(X)
        weights = W * resp[:, 0]
        self.is_density.mean[0] = np.average(X, axis=0, weights=weights)
        self.is_density.cov[0] = (X - self.is_density.mean[0]).T @ np.diag(weights / np.sum(weights)) @ (
                X - self.is_density.mean[0])
        self.is_density.weights[0] = np.average(resp[:, 0], weights=W)


class ImprovedCEAISGaussianMixture(ImprovedCEAIS):
    def initialize_is_density(self):
        """ This function initializes the importance sampling density. """
        self.is_density = GaussianMixtureDensity(p=self.input_density.p, K=2,
                                                 weights_init=[0.5, 0.5],
                                                 mean_init=np.array([[-0.1, 0], [0.2, 0]]),
                                                 eigenvalue_bound=[0.01, 100])
...
class ImprovedCEAISKernelDensityEstimation(ImprovedCEAIS):
    def __init__(self, limit_state_func, input_density):
        super().__init__(limit_state_func=limit_state_func, input_density=input_density)
        self.opt_bdw = 1
    ...
    def update_is_density(self, X, g_X, sigma):
        ...
        # use cross validation to find the optimal bandwidth.
        self.opt_bdw = self.cv_bandwidth_selection(X, W, self.opt_bdw)

        # construct a KDE with failure samples.
        self.is_density = KernelDensityEstimation(X, w=W, bandwidth=self.opt_bdw)

    @staticmethod
    def cv_bandwidth_selection(X, W, opt_bdw_upper_bound):
        """ This function selects the optimal eigenvalue lower bound for the Gaussian Mixture Density. """
        ...
        def objective(bdw):
            ...
            for train_index, test_index in kf.split(X):
                X_train, X_test = X[train_index], X[test_index]
                W_train, W_test = W[train_index], W[test_index]
                kde = KernelDensityEstimation(X_train, w=W_train, bandwidth=bdw)
                llh += kde.weighted_llh(X_test, data_weights=W_test)

            optimization_steps.append((bdw, -llh / num_fold))
            # print(f"Trial {len(optimization_steps)}, "
            #       f"eigenvalue Lower Bound: {bdw}, Average Log-Likelihood: {llh / num_fold}")
        ...
        result = minimize_scalar(objective,
                                 method='bounded',
                                 bounds=(0.01, opt_bdw_upper_bound),
                                 options={'xatol': 2e-3})
        # print("The optimal eigenvalue lower bound is {}".format(result.x))
